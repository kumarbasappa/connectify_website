    ...
    >
      ...
      <div className="flex items-center justify-center h-7 w-auto min-w-[80px] max-w-[125px]">
        ...
        <img
          src={logoSvg}
          alt={`${name} Logo`}
          className="h-full w-auto max-h-7 object-contain grayscale opacity-60 hover:grayscale-0 hover:opacity-100 dark:brightness-0 dark:invert dark:opacity-60 dark:hover:opacity-100 transition-all duration-300"
          loading="eager"
        />
      </div>
      ...
      {/* Domain Badge */}
      <span className={`text-xs font-semibold tracking-wide uppercase px-2 py-0.5 rounded-full border ${badgeStyle}`}>
        {industry}
      </span>
    </motion.div>
  );
  ...
  return (
    <section className="relative overflow-hidden border-y border-black/10 bg-background py-12 dark:border-white/10">
      <div className="mx-auto max-w-7xl px-0 text-center">
        <p className="font-mono text-xs font-bold tracking-[0.25em] text-slate-900 dark:text-cyan-400">
