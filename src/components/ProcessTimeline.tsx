  ...
  return (
    <section className="relative bg-surface py-24 lg:py-32 border-t border-black/5 dark:border-white/10">
      <div className="mx-auto w-full max-w-7xl px-6 lg:px-12">
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-start rounded-2xl border p-6 text-left transition-all duration-300 ease-in-out ${
                  isActive
                    ? "border-amber-400 bg-card-bg shadow-[0_0_25px_rgba(245,158,11,0.25)] scale-[1.02]"
                    : "border-black/10 bg-card-bg hover:border-amber-400/60 hover:shadow-[0_0_20px_rgba(245,158,11,0.15)] dark:border-white/10"
                }`}
              >
                <div className="flex w-full items-center justify-between">
                  <span
                    className={`font-mono text-2xl font-black transition-colors ${
                      isActive ? "text-amber-400" : "text-slate-500 opacity-60"
                    }`}
                  >
                    {step.stepNumber}
                  </span>
                ...
                <h3 className="font-display mt-4 text-lg font-bold text-foreground dark:text-white">
                  {step.title}
                </h3>
              </button>
            );
        ...
        <div className="mt-8 rounded-3xl border border-black/10 bg-card-bg p-8 shadow-xl transition-all duration-300 hover:border-amber-400/60 hover:shadow-[0_0_25px_rgba(245,158,11,0.2)] lg:p-12 dark:border-white/10">
          <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
            <div>
              <span className="rounded-full bg-amber-500/10 border border-amber-500/30 px-4 py-1.5 font-mono text-xs font-bold text-amber-400">
                STAGE {processSteps[activeStep].stepNumber}
              </span>
              <h3 className="mt-4 font-display text-2xl font-extrabold text-foreground sm:text-4xl dark:text-white">
                {processSteps[activeStep].title}
              </h3>
              <p className="font-display mt-2 text-base font-semibold text-amber-400">
          ...
          <div className="mt-10 grid gap-8 border-t border-black/10 pt-8 md:grid-cols-2 dark:border-white/10">
            <div>
              ...
              <ul className="mt-4 space-y-2.5">
                {processSteps[activeStep].activities.map((act, i) => (
                  <li key={i} className="flex items-center gap-3 font-sans text-sm font-medium text-slate-700 dark:text-slate-300">
                    <svg className="h-4 w-4 text-amber-400 flex-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
